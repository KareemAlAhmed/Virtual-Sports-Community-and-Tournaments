...
export default {
    ...
    methods: { 
           ...
           return moment(String(value)).format('MMM DD,YYYY')
      ...
      },postLiked(postId,userId,listUsers,isLiked){
            ...
            if(userId != null){
                    ...
                    for (const key in users) {
                        if(parseInt(users[key])==userId){                           
                            userLiked=true;
                            break;
                        }
                    }   
                     if(!this.reaction.isLiked && userLiked != true){
                        let situation='Liked'
                        store.dispatch("likeOrUnlikePost",{postId,userId,situation})
      ...
      },userLiked(postId,userId){

            axios.get("http://127.0.0.1:8000/api/post/"+postId+"/getLikes")
                .then(re =>{
                    this.likedUser=re.data
                    let userLiked = false;                   
                    for (const key in this.likedUser) {
                        if(parseInt(this.likedUser[key])==userId){                           
                            userLiked=true;
                            break;
                        }
                    }
                    this.userLike=userLiked;
            })
